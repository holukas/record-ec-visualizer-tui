...
from record_ec_visualizer_tui.model import WIND_RAW_KEYS, LiveState
...
class VisualizerApp(App[None]):
    ...
    def on_mount(self) -> None:
        self._reader = self.run_worker(self._consume(), name="stream-reader", exclusive=True)
    ...
    def _refresh(self) -> None:
        ...
        self.query_one("#status", StatusBar).update_from(state, self.paused)
...
def _append_health(text: Text, label: str, health: object) -> None:
    age = health.age()  # type: ignore[attr-defined]
    stale = health.is_stale  # type: ignore[attr-defined]
    text.append(f"{label} ", style="grey50")
    if age is None:
        text.append("no data", style="red")
        return
    text.append(f"{age:4.1f}s ago", style="red" if stale else "green")
    errors = health.errors  # type: ignore[attr-defined]
    if errors:
        text.append(f" ({errors} bad)", style="yellow")
